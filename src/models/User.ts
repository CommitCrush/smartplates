...
import { User, CreateUserInput, UpdateUserInput, PublicUserProfile } from '@/types/user';
 ...
 */
export async function updateUser(userId: string | ObjectId, updateData: UpdateUserInput): Promise<User | null> {
  try {
    const usersCollection = await getCollection<User>(COLLECTIONS.USERS);
 ...
 */
export async function getAllUsers(limit: number = 50, skip: number = 0): Promise<User[]> {
  try {
    const usersCollection = await getCollection<User>(COLLECTIONS.USERS);
 ...
 */
export async function getPublicUserProfile(userId: string | ObjectId): Promise<PublicUserProfile | null> {
  try {
    const usersCollection = await getCollection<User>(COLLECTIONS.USERS);
 ...
 */
export async function saveRecipeForUser(userId: string | ObjectId, recipeId: string | ObjectId): Promise<boolean> {
  try {
    const usersCollection = await getCollection<User>(COLLECTIONS.USERS);
 ...
 */
export async function removeSavedRecipeForUser(userId: string | ObjectId, recipeId: string | ObjectId): Promise<boolean> {
  try {
    const usersCollection = await getCollection<User>(COLLECTIONS.USERS);
